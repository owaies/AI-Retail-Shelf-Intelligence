...
export function SettingsPage() {
  ...
  const save = () => { sessionStorage.setItem('retail_vision_access_token', token.trim()); setSaved(true); setTimeout(() => setSaved(false), 1800) }
  return <><PageHeader index="05" eyebrow="SYSTEM SETTINGS" title="Control the workspace." description="Authentication is handled by Supabase Auth. The access token stays in this browser session and is never committed to the repository." /><section className="panel settings-panel"><div className="setting-row"><div><strong>API endpoint</strong><span>Configured with VITE_API_BASE_URL, defaulting to /api</span></div><code>/api</code></div><div className="setting-row"><div><strong>Supabase Auth</strong><span>Use the secure login screen for access tokens.</span></div><Link className="secondary-button" to="/login">OPEN LOGIN</Link></div><div className="setting-row"><div><strong>Vision model</strong><span>YOLOX-S · official ONNX runtime integration · higher-capacity baseline</span></div><code>0.1.1RC0</code></div><div className="setting-row"><div><strong>Detection threshold</strong><span>Recall-oriented baseline threshold; evaluation is required before claiming accuracy.</span></div><code>0.20</code></div><div className="setting-row"><div><strong>Database</strong><span>retail_shelf_intelligence schema · user-scoped records</span></div><code>ISOLATED</code></div><div className="setting-row"><div><strong>Session token</strong><span>Optional manual token entry for local testing only.</span></div><input value={token} onChange={e => setToken(e.target.value)} type="password" autoComplete="off" placeholder="Paste access token" aria-label="Access token" /><button className="secondary-button" type="button" onClick={save}>{saved ? 'SAVED' : 'SAVE SESSION'}</button></div></section></>
}